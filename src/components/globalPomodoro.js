export function injectGlobalPomodoroUI() {
  ...
  style.innerHTML = `
    ...
    /* HIDE DEFAULT BROWSER ARROWS IN NUMBER INPUTS */
  ...
  wrapper.innerHTML = /* html */`
    <button id="open-pomodoro-btn" class="fixed bottom-8 right-8 z-[100] h-12 px-5 bg-[#0b0612]/90 backdrop-blur-xl rounded-2xl border border-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.5)] flex items-center justify-center gap-3 group hover:border-fuchsia-500/30 hover:bg-[#130b1c]/90 transition-all duration-300 cursor-pointer">
    ...
    <div id="pomodoro-modal" class="hidden fixed inset-0 z-[200] items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
       <div class="bg-[#0b0612] border border-white/5 w-full max-w-[380px] rounded-[2.5rem] overflow-hidden shadow-[0_0_80px_rgba(217,70,239,0.05)] relative animate-in zoom-in-95 duration-500">
          
          <button id="close-pomodoro-modal" class="absolute top-6 right-6 text-zinc-500 hover:text-white transition-colors z-10">
          ...
          <div class="p-8 flex flex-col items-center">
             ...
             <div class="relative w-56 h-56 flex items-center justify-center mb-6 bg-[#130b1c] rounded-full shadow-[inset_0_0_40px_rgba(0,0,0,0.8)]">
                <svg class="absolute inset-0 w-full h-full -rotate-90 drop-shadow-[0_0_15px_rgba(217,70,239,0.4)]">
                   <circle cx="112" cy="112" r="106" stroke="currentColor" stroke-width="3" fill="transparent" class="text-white/[0.03]"></circle>
                ...
                <div class="relative z-10 flex flex-col items-center">
                   <span id="pomo-timer-display" class="text-6xl font-black text-white tracking-tighter tabular-nums drop-shadow-[0_0_15px_rgba(255,255,255,0.2)] mt-2">25:00</span>
                   <span class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Minutes</span>
                </div>
             </div>
             ...
             <div class="flex items-center justify-center gap-3 mb-6 w-full px-2">
                <div class="relative flex-1 flex items-center bg-white/5 border border-white/10 rounded-xl p-1 focus-within:border-fuchsia-500/50 transition-colors">
                   <button class="w-8 h-8 rounded-lg flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/10 transition-colors pomo-minus-btn" data-target="global-custom-time">
                      <i class="fa-solid fa-minus text-xs pointer-events-none"></i>
                   </button>
                   <input type="number" id="global-custom-time" min="1" max="300" value="25" class="bg-transparent text-white font-black text-lg w-full outline-none text-center tabular-nums hide-spinner">
                   <button class="w-8 h-8 rounded-lg flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/10 transition-colors pomo-plus-btn" data-target="global-custom-time">
             ...
             <div class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-8 flex items-center gap-2">
                Reward: <span id="global-pomo-reward" class="text-amber-400 font-black">+50 XP</span>
             </div>

             <div class="flex items-center gap-4 w-full">
                <button id="pomo-reset" class="w-14 h-14 shrink-0 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 flex items-center justify-center text-zinc-400 hover:text-white transition-all">
